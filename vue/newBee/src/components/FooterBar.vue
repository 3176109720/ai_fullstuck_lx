<template>
    <van-action-bar>
        ...
        <van-action-bar-icon icon="cart-o" text="购物车" :badge="badgeNum" v-if="badgeNum" />
...
onMounted(() => {
    store.dispatch('setCartCountAction') // store 中 actions 里面的元素都要用dispath调用
})

const handleAddCart = async () => {
    const res = await addToCart({ goodsCount: 1, goodsId: props.id })
